...
| Integration Apps          | Hosted server, setup/auth URLs can receive `uid`; endpoints should return quickly.            | Mostly covered | We have setup UI, `/api/setup-status`, `/api/health`. Mobile App Home opens with `uid`; current macOS Open can open the bare home URL, so the page now remembers/restores the last UID. We are not using trigger webhooks in v1. |
